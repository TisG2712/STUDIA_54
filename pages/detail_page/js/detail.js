...
// Hiệu ứng 2: Fade-in cho ảnh nền khi đã sẵn sàng.
function initBackgroundImage() {
  const image = document.getElementById("bg-image");
  if (image) {
    // Thêm class is-loaded ngay lập tức để ảnh hiển thị
    image.classList.add("is-loaded");

    // Thêm event listener để xử lý khi ảnh load xong
    image.addEventListener("load", function () {
      image.classList.add("is-loaded");
    });

    // Thêm event listener để xử lý lỗi ảnh
